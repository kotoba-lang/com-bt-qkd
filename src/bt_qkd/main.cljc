(ns bt_qkd.main
  "Kotodama WASM entrypoint for the bt_qkd clean-room actor (L4) — Clojure port.
  ...
  facts (`bt_qkd.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bt_qkd")
(def tier "L4")
...
(def entity-specs
  [{:entity "KeySession" :plural "keysessions" :id-prefix "btqkd_key"
   ...
   {:entity "Protocol"   :plural "protocols"   :id-prefix "btqkd_pro"
    :fields [:name :family]                    :required [:name :family]
    :coerce {}                                  :refs {}}
   {:entity "Alarm"      :plural "alarms"      :id-prefix "btqkd_ala"
...
(defn emit-facts
  "EAVT facts for one record: {\"bt_qkd.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bt_qkd-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bt_qkd-compat/ready)
